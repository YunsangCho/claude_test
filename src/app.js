...
const SUBJ=[].concat(RAW_SUBJ_A,RAW_SUBJ_B,RAW_SUBJ_C,RAW_SUBJ_D,RAW_SUBJ_E,RAW_SUBJ_F,RAW_SUBJ_G,RAW_SUBJ_H,RAW_SUBJ_I,RAW_SUBJ_J,RAW_SUBJ_K,RAW_SUBJ_L).map((r,i)=>({id:"u"+i,ch:r[0],name:r[1],era:r[2],year:r[3],facts:r[4],relics:r[5]||[],hint:r[6]||""}));
const HERI=[].concat(RAW_HERI_A,RAW_HERI_B,RAW_HERI_C,RAW_HERI_D,RAW_HERI_E,RAW_HERI_F,RAW_HERI_G,RAW_HERI_H,RAW_HERI_I,RAW_HERI_J,RAW_HERI_K).map((r,i)=>({id:"w"+i,ch:r[0],name:r[1],era:r[2],kind:r[3],facts:r[4]}));
const EVT=[].concat(RAW_EVENT_A,RAW_EVENT_B,RAW_EVENT_C,RAW_EVENT_D,RAW_EVENT_E,RAW_EVENT_F,RAW_EVENT_G,RAW_EVENT_H,RAW_EVENT_I).map((r,i)=>({id:"v"+i,ch:r[0],name:r[1],year:r[2],era:r[3]}));
SUBJ.forEach(x=>x.cat="s");HERI.forEach(x=>x.cat="h");
const IMGS=[].concat(RAW_IMG,RAW_IMG_B).map((r,i)=>({id:"g"+i,ch:r[0],name:r[1],group:r[2],ask:r[3],svg:r[4],tip:r[5]||""}));
